...
class AgentFactory:
    ...
    def create_new_agent(env, agent_type=settings.AGENT_TYPE):
        ...
        if agent_type == "ddqn":
            return DDQNAgent(state.size, env.action_space.n, batch_size=32,
                             update_target_network_frequency=50, lr=0.0005, gamma=0.8)
        if agent_type == "ppo":
            return PPOAgent(state.size, env.action_space.n)
        if agent_type == "ddpg":
            return DDPGAgent(state.size, env.action_space.shape[0], env, noise=0.8)
